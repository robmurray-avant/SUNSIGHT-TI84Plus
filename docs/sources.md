...
Murdoch's eight operational TI-81 programs total **2,259 bytes** in the published listing; the separate 128-byte `ALLOUT` diagnostic program brings all nine listings to 2,387 bytes. SUNSIGHT v1.0 occupied **6,761 bytes on the calculator**. SUNSIGHT v1.1 occupies **6,997 bytes on the calculator**; its `.8xp` file is **7,056 bytes**. The two calculators use different tokenization and storage architectures, so byte counts are useful context rather than a precise measure of algorithmic complexity.
